...
The Fulcrum stage runs through `runFulcrumStage()` in `V2_emailSender.js`, which picks one of two paths (apiMode/dryRun default to env but a Lambda event can override per-invocation via `event.fulcrumApiMode` / `event.fulcrumApiDryRun`):
- **`FULCRUM_API_MODE` (now the prod default — set to `'1'` in `template.yaml`):** `runFulcrumApiMode` (`fulcrumInvoiceApi.js`) — logs in for the session, then discovers (`GetInvoiceGridDataQuery`), creates (`CreateInvoiceFromSalesOrderCommand`), and issues (`POST /api/invoices/{id}/status {issued}` on the public Bearer API) entirely via HTTP — no clicking/Blazor waits, parallelizable, uncapped (no `FULCRUM_MAX_ACTIONS`). `FULCRUM_API_DRY_RUN=1` plans without writing. Revert to the browser path by setting `FULCRUM_API_MODE` to `''`.
- **Browser fallback:** `runFulcrumProcessor` — Puppeteer DOM automation (`fulcrumProcessor.js`). Skip rules (refunds/zero-value) reuse the same `shouldProcessRow`, so they're identical to the browser path. Needs SSM read on `/rsg-ai/prod/fulcrum-api-key` (granted in `template.yaml`). Verified equivalent (issues + triggers QBO sync) in specs/013 Phase 0.

After the Fulcrum stage issues invoices, `runFulcrumStage` is followed by `waitForFulcrumQboSync` so the async Fulcrum→QBO sync registers the new invoices before the QBO send stage queries "unsent unpaid" (otherwise just-issued invoices slip to the next run — the silent gap that dropped F10488/F10489 on 2026-06-23, specs/015). API mode captures each issued invoice's `DocNumber` (`fulcrumInvoiceDocNumber`, e.g. "F10488"), so the wait is a **bounded poll**: it re-checks QBO (cheap `invoiceModule.findInvoicesByDocNumbers` → `DocNumber IN (...)`) every `FULCRUM_QBO_SYNC_WAIT_MS` (default 20s) until all issued invoices are visible, up to `FULCRUM_QBO_SYNC_MAX_WAIT_MS` (default 120s); on timeout it proceeds and reconciliation flags the gap. Browser mode (no captured DocNumbers) and "nothing issued" fall back to the legacy single fixed wait / skip. Note: Fulcrum→QBO auto-email-on-sync is OFF, so our QBO send stage is the sole emailer and exclusions/allowlist are respected.

**Issued-vs-sent reconciliation (specs/015):** because "8 created" (Fulcrum) and "N sent" (QBO) were reported independently, a gap used to be invisible. `buildSummaryEmailContent` now runs pure exported `reconcileIssuedVsSent({ processedInvoices, details })` — issued invoices QBO never even fetched (not sent/skipped/errored; those already surface in their own sections) become an **ACTION REQUIRED** line naming the F-number + SO, which also bumps the "needs attention" count. The stale-email angle is a *symptom* of this same gap, not a separate bug: the send stage overwrites `BillEmail` with the freshly-resolved recipient (QBO customer `PrimaryEmailAddr` / HLI ship-to routing) **before** sending, so an unprocessed invoice simply keeps its old Fulcrum-synced address — processing it refreshes the recipient.

### Send-time tracking-number guard (and the Will Call exception, specs/014)
The QBO send stage requires a carrier tracking number per invoice: `externalDataModule.fetchExternalDataForInvoice` throws "Could not determine a trackingNumber" when the chosen Fulcrum shipment has none, which deterministically blocks that invoice on **every** run. Customer-pickup / **"Will Call"** orders legitimately have no tracking number, so a config-driven exception records the shipping-method name (e.g. `"Will Call"`) in QBO's `TrackingNum` instead of failing. The rule is an explicit **(customer, shipping-method) allowlist** — `TRACKING_OPTIONAL_RULES` + pure `trackingPlaceholderForOrder()`/`resolveTrackingNumber()` in `V2_emailSender.js` (exported, tested) — so adding a customer/method is a one-line edit and the hard error is preserved for everyone else. Initial entry: HLI Solutions + "Will Call". (Sibling over-strict guard: the same-date shipment guard in `chooseShipment`, specs/011.)

### Testable exports
`V2_emailSender.js` exports `buildFulcrumRunOptions`, `buildInvocationLockMetadata`, `buildSummaryEmailContent`, `customerModule`, `utils`, `externalDataModule`, the tracking helpers (`resolveTrackingNumber`, `trackingPlaceholderForOrder`, `TRACKING_OPTIONAL_RULES`), and the reconciliation helpers (`reconcileIssuedVsSent`, `normalizeDocNumber`); `fulcrumProcessor.js` exports `isCreateDetailTimeoutError`; `fulcrumInvoiceApi.js` exports `fulcrumInvoiceDocNumber`. The regression suite (`tests/invoiceSender.test.js`) exercises these to protect: excluded-customer visibility in summary emails, HLI ship-to routing, default primary-email recipient selection, the Will Call tracking exception, the same-date shipment guard, and the issued-vs-sent reconciliation (issued-but-not-sent invoices surface in ACTION REQUIRED). When changing send/routing/summary logic, extend these tests and keep them green.
