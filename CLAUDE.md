...
After the Fulcrum stage issues invoices, `runFulcrumStage` is followed by `waitForFulcrumQboSync` — a short delay (default 20s, `FULCRUM_QBO_SYNC_WAIT_MS`, skipped if nothing was issued) so the async Fulcrum→QBO sync registers the new invoices before the QBO send stage queries "unsent unpaid" (otherwise just-issued invoices slip to the next run). Note: Fulcrum→QBO auto-email-on-sync is OFF, so our QBO send stage is the sole emailer and exclusions/allowlist are respected.
